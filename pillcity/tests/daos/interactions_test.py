...
from pillcity.daos.comment import create_comment, delete_comment, dangerously_get_comment
from pillcity.daos.reaction import create_reaction, get_reaction
...
class InteractionsTest(BaseTestCase):
    def _assert_user_to_post_privilege(
        ...
        comment1 = None
        if comments:
            comment1 = create_comment(acting_user, 'comment1', post, None, [], [], None)
            ...
            def op1():
                create_comment(acting_user, 'comment1', post, None, [], [], None)

            self.assertRaises(UnauthorizedAccess, op1)
            comment1 = create_comment(post.author, 'comment1', post, None, [], [], None)

        if nested_comments:
            nested_comment1 = create_comment(acting_user, 'nested_comment1', post, comment1, [], [], None)
            ...
            def op2():
                create_comment(acting_user, 'nested_comment1', post, comment1, [], [], None)

            self.assertRaises(UnauthorizedAccess, op2)
            nested_comment1 = create_comment(post.author, 'nested_comment1', post, None, [], [], None)
